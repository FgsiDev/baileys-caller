...
const DEBUGGER_URL =
  process.env.CALL_CHROME_DEBUGGER_JSON_URL ||
...
async function main() {
  ...
  const page = targets.find(
    (t) => t.type === "page" && t.url?.includes("web.whatsapp.com"),
  );
  if (!page?.webSocketDebuggerUrl)
    throw new Error("No WhatsApp Web page found");
  ...
  if (resourceInfo.error)
    throw new Error(`Browser eval failed: ${resourceInfo.error}`);
  if (!resourceInfo.workerUrl)
    throw new Error("Worker URL not found in browser");
