...
const resolveWorkerScriptPath = () => {
  ...
  return fs.existsSync(compiled)
    ? compiled
    : path.join(__dirname, "worker-bootstrap.mts");
};
  ...
  constructor(worker, name = "WAWebVoipWebWasmWorker") {
    ...
    if (typeof worker.on === "function") {
      ...
      worker.addEventListener("message", (ev) =>
        this.#handleMessage(ev?.data ?? ev),
      );
    }
  }
  postMessage = (msg, transferList) => {
    const out =
      msg && typeof msg === "object" && msg.cmd && !msg.type
        ? { ...msg, type: "cmd" }
        : msg;
...
export class WasmEngine {
  ...
  initialize = async () => {
    ...
    const diskWorkerCode = fs.existsSync(workerFile)
    ...
    const workerBundleHasLoader =
      typeof this.#workerModulesCode === "string" &&
      /WAWebVoipWebWasmLoader/.test(this.#workerModulesCode);
    ...
    this.#vmContext.WAWebVoipWebWasmWorkerResource = this.#requireModule(
    ...
    if (typeof wasmLoader !== "function") {
      throw new Error(
        `No compatible WASM loader found. Tried: ${loaderModuleNames.join(", ")}`,
      );
    }
  ...
  handleOnTransportMessage = (data, ip, port) => {
    ...
    if (typeof this.#instance.handleOnMessageFromHeap === "function") {
      ...
      try {
        const heapU8 =
          this.#instance.GROWABLE_HEAP_U8?.() ?? this.#instance.HEAPU8;
        if (!heapU8) return;
        heapU8.set(data, ptr);
  ...
  #startAudioPlaybackLoop = () => {
    ...
    this.#audioPlaybackLoopInterval = setInterval(() => {
      ...
      try {
        this.#instance.requestAudioDataFromWasmVoip(
    ...
    new Promise((resolve) => {
      const loadedHandler = (msg) => {
        if (msg && msg.cmd === "loaded") {
          ...
          if (
            this.#workersLoadedCount >= PTHREAD_POOL_SIZE &&
  ...
  #loadWasmModuleToAllWorkers = async () => {
    ...
    await Promise.all(
      this.#unusedWorkers.map((w) => this.#loadWasmModuleToWorker(w)),
    );
  };
  #registerGlobalCallbacks = () => {
    ...
    _a.registerGlobalCallbackListener("startCaptureJS", () =>
      callbacks.onAudioCaptureStart?.(),
    );
    _a.registerGlobalCallbackListener("stopCaptureJS", () =>
  ...
  #requireModule = (name) => {
    ...
    if (
      ...
      try {
        ...
        if (
          typeof legacyResult === "function" ||
          (legacyResult && Object.keys(legacyResult).length > 0)
        ) {
          result = legacyResult;
        }
      } catch {}
  ...
  #createVMContext = (memory) => {
    ...
    const wasmCallbacks = {
      ...
      cryptoHkdfExtractWithSaltAndExpand: (data) => {
        ...
        return (
          callbacks.cryptoHkdf?.(key, salt, info, length) ??
