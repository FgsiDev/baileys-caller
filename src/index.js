...
const computeHkdf = (key, salt, info, length) => {
  const effectiveSalt =
    salt && salt.length > 0 ? Buffer.from(salt) : Buffer.alloc(SHA256_LEN, 0);
...
export class VoipClient {
  ...
  constructor(config) {
    ...
    this.#baileys = this.#config.baileys;
  }
  ...
  connect = async () => {
    ...
    this.#relay = new RelayRtcTransport({
      onTransportMessage: (data, ip, port) =>
        this.#engine?.handleOnTransportMessage(data, ip, port),
    ...
    this.#engine = new WasmEngine({
      callbacks: {
        ...
        onCallEvent: (eventType, eventData) =>
          this.#handleCallEvent(eventType, eventData),
        ...
        onAudioPlaybackData: (audioData) =>
          this.#activeCall?._emitAudio(audioData),
    ...
    this.#sock.ws.on("CB:call", (node) => {
      this.#signaling.processIncomingCall(
        node,
        this.#engine,
    ...
    this.#sock.ws.on("CB:receipt", (node) => {
      ...
      this.#signaling.processIncomingReceipt(
        node,
        this.#engine,
        this.#activeCall?.callId ?? "",
      );
  ...
  call = async (phoneNumber, opts = {}) => {
    ...
    const deviceList = peerDeviceJids.length
      ? peerDeviceJids
      : [toBareJid(peerLid)];
    ...
    const callId = (
      "00" + randomBytes(16).toString("hex").slice(2)
    ).toUpperCase();
