...
if (
  typedWorkerData &&
  (typedWorkerData.loaderCode || typedWorkerData.workerModulesCode)
) {
  try {
    ...
    if (typedWorkerData.workerModulesCode) {
      const workerScript = new vm.Script(
        modulePrelude + typedWorkerData.workerModulesCode,
        {
    ...
    if (typedWorkerData.loaderCode) {
      const loaderScript = new vm.Script(
        modulePrelude + typedWorkerData.loaderCode,
        {
    ...
    try {
      ...
      if (jsWorkerModule) {
        WAWebVoipJsWorkerMessageHandler =
          jsWorkerModule.default ?? jsWorkerModule;
      }
    } catch (e) {}
  } catch (e) {}
}
...
const handleRawVideoFrameOnJsWorker = (msg) => {
  ...
  if (
    !jsWorkerRawVideoFramePtr ||
    jsWorkerRawVideoFrameSize < bytes.byteLength
  ) {
...
e.addMessageListener("waWasmWorkerCompatibleCallback", function (msg) {
  ...
  if (
    typeof global.self.WhatsAppVoipWasmWorkerCompatibleCallbacks[
      callbackName
    ] !== "function"
  )
    return;
