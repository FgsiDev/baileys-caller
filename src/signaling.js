...
export class SignalingBridge {
  ...
  init = async () => {
    ...
    const originalKeysSet = this.#sock.authState.keys.set.bind(
  ...
  requestTcToken = async (jid) => {
    ...
    try {
      ...
      for (const tokenNode of tokenNodes) {
        ...
        if (
          this.#baileys.jidNormalizedUser(tokenJid) !==
          this.#baileys.jidNormalizedUser(userJid)
        )
          continue;
        ...
        if (content instanceof Uint8Array && content.length > 0) {
          ...
          await this.#sock.authState.keys.set({
            tctoken: {
              [userJid]: { token, timestamp: String(tokenNode.attrs.t ?? "") },
            },
          });
          return token;
        }
  ...
  ensureTcToken = async (...jids) => {
    const uniqueJids = [
      ...new Set(
        jids
          .map((j) => this.#toBareJid(String(j ?? "").trim()))
          .filter(Boolean),
      ),
    ];
    ...
    for (const jid of uniqueJids) {
      const fetched = await Promise.race([
        ...
        new Promise((r) =>
          setTimeout(() => r(undefined), TC_TOKEN_REQUEST_TIMEOUT_MS),
        ),
      ]);
  ...
  discoverPeerDevices = async (peerLidJid) => {
    ...
    return this.#normalizeStartCallPeerList(
      devices.map((d) => d.jid).filter(Boolean),
    );
  };
  ...
  #doSendSignaling = async (peerJid, callId, xmlPayload) => {
    ...
    let voipNode;
    try {
      voipNode = await decodeBinaryNode(
        Buffer.concat([Buffer.from([0]), rawPayload]),
      );
    } catch {
    ...
    if (destination) {
      ...
      if (sessionTargets.length)
        await this.#ensureSignalSessions(
          sessionTargets,
          signalingTag === "offer",
        );
      ...
      for (const destNode of destinations) {
        ...
        if (!targetJid || !destEnc || !(destEnc.content instanceof Uint8Array))
          continue;
        try {
          const encrypted = await this.#encryptCallKey(
            targetJid,
            destEnc.content,
            encCount,
          );
  ...
  #sendCallStanza = async (
    ...
    void (async () => {
      try {
        const ackNode = await this.#sock.waitForMessage(
          stanzaId,
          ACK_TIMEOUT_MS,
        );
        if (!ackNode || !this.#voip) return;
        const { encodeBinaryNode } = this.#baileys;
        const ackPayload = Buffer.from(encodeBinaryNode(ackNode)).toString(
          "base64",
        );
        const tcToken = await this.ensureTcToken(
          effectivePeerJid,
          callbackPeerJid,
        );
        try {
  ...
  #doProcessIncomingCall = async (node, voip, activeCallId) => {
    const { getAllBinaryNodeChildren, getBinaryNodeChild, encodeBinaryNode } =
      this.#baileys;
    const voipChild = getAllBinaryNodeChildren(node)[0];
    if (!voipChild) return;
    const incomingCallId = String(
      voipChild.attrs["call-id"] ?? voipChild.attrs.call_id ?? "",
    );
    ...
    const storedPeerJid = callIdForRouting
      ? this.#incomingCallPeerById.get(callIdForRouting)
      : undefined;
    ...
    if (callIdForRouting && (callbackPeerJid || senderDeviceJid)) {
      this.#remoteXmppRoutePeerByCallId.set(
        callIdForRouting,
        callbackPeerJid || senderDeviceJid,
      );
      const hinted = this.#pickConcreteRouteHint(
        senderDeviceJid,
        callbackPeerJid,
      );
    ...
    const routedPeerJid =
      usableNode.tag === "offer"
        ? this.#preferDeviceRouteJid(
            senderDeviceJid,
            callbackPeerJid,
            storedPeerJid,
          )
        : this.#preferOrderedRouteJid(
            mappedRemoteDeviceJid,
            storedPeerJid,
            senderDeviceJid,
            callbackPeerJid,
          );
    ...
    switch (usableNode.tag) {
      ...
      default:
        ...
        if (
          callIdForRouting &&
          (usableNode.tag === "terminate" || usableNode.tag === "reject")
        ) {
  ...
  #doProcessIncomingReceipt = async (node, voip, activeCallId) => {
    ...
    const incomingCallId = String(
      receiptChild.attrs["call-id"] ?? receiptChild.attrs.call_id ?? "",
    );
    const callIdForRouting = incomingCallId || activeCallId;
    if (activeCallId && incomingCallId && incomingCallId !== activeCallId)
      return;
    const callbackPeerJid = String(
      node.attrs.from ?? receiptChild.attrs["call-creator"] ?? "",
    );
    const storedPeerJid = callIdForRouting
      ? this.#incomingCallPeerById.get(callIdForRouting)
      : undefined;
    const routedPeerJid = this.#preferOrderedRouteJid(
      storedPeerJid,
      callbackPeerJid,
    );
    if (callIdForRouting && routedPeerJid)
  ...
  #maybeDecryptEnc = async (voipNode, peerJid) => {
    ...
    const candidates = [
      ...new Set([peerJid, this.#toCallDeviceJid(peerJid)]),
    ].filter(Boolean);
    let lastErr;
    ...
    for (const jid of [...new Set(jids.filter(Boolean))]) {
      ...
      if (!refresh) {
        const validation =
          await this.#sock.signalRepository.validateSession(jid);
